...
| 4 | `session-index.md` ≤200 lines AND contains exactly the last 10 sessions (by date header count) | `wc -l AGENT_DIR/session-index.md` ≤200 AND `grep -cE '^## 2026-' AGENT_DIR/session-index.md` returns ≤10 |
...
| 2 | `patterns/reference/` exists and contains every pattern not fired in 60 days | `ls AGENT_DIR/patterns/reference/*.md \| wc -l` ≥11 (the demoted ones from the 21 starting count) |
| 3 | Each demoted pattern's frontmatter has `last_fired:` ≥60 days old (verifying demotion was earned, not arbitrary) | `for f in AGENT_DIR/patterns/reference/*.md; do grep last_fired $f; done` — every entry's date is on or before 2026-03-08 |
