...
function setFrontmatterField(content, key, value) {
  ...
  if (presentRe.test(newBody)) {
    // Codex R2: the prior `(\\S*)(.*)$` capture preserved the tail of a
    // multi-word existing value (e.g., `demoted_reason: old text here`),
    // leaking it into the new line. Match the WHOLE rest-of-line and
    // explicitly split off any inline `# comment` so the value is replaced
    // atomically while comments survive.
    const replaceRe = new RegExp(
      `^([ \\t]*${escapeRegExp(key)}[ \\t]*:)([^\\r\\n]*)$`,
      'm',
    );
    newBody = newBody.replace(replaceRe, (_, lead, rest) => {
      const commentMatch = rest.match(/[ \t]+#.*$/);
      const comment = commentMatch ? commentMatch[0] : '';
      return `${lead} ${value}${comment}`;
    });
  } else {
...
function stampDemotion(content, today, reason) {
  // Per ADR 008: `demotion_phase` is reserved for SCOPED MASS CLEANUPS
  // (e.g., the 2026-05-09 one-time bootstrap) — not for nightly worker
  // demotions, which satisfy criterion (a) directly via the firing-log
  // demotion gate. Stamping `demotion_phase: p3-<today>` here would conflict
  // with the frozen historical marker on date 2026-05-09 and crash the
  // worker. The validateDemotionPhase + canRePromote helpers remain in
  // bootstrap-guard.js for future scoped-cleanup tooling. Reality-checker R2.
  let out = content;
...
async function planJournal({ memoryRoot, today }) {
  ...
  // overwrite the archive copy. If contents differ → record `archiveCollision`
  // sentinel in the plan rather than throwing — other prune steps (corrections,
  // session-index, demotions) still complete and the dream-log surfaces the
  // collision for JJ. Reality-checker R2 finding: throwing nuked the whole
  // staging; partial progress is better than total rollback.
...
async function planDemotions({ memoryRoot, firingEntries, days, gracePeriodDays, today, now, reinforcedNames = [] }) {
  const active = await loadPatterns(memoryRoot, 'active');
  if (active.length === 0) return [];

  // Production-bug fix (2026-05-11): when pattern-firing-log.md is empty or
  // missing (the wrap-up → firing-log writer hook isn't wired yet for a
  // given consumer), `firingEntries.length === 0`. demotionCandidates would
  // then return EVERY active pattern (none have "fired" within the window
  // because nothing has been recorded). Demoting 9/10 patterns wholesale on
  // the first nightly run is catastrophic. Safety bias: defer ALL demotion
  // until the firing-log produces evidence. Surface this gap via the
  // dream-log + Stage A C6 freshness warn (already wired).
  if (!Array.isArray(firingEntries) || firingEntries.length === 0) {
    return [];
  }
  ...
  // Phase 2 ↔ Phase 3 collision guard: if Phase 2 just reinforced a pattern
  // in this same run, that's fresh evidence the rule fired even if it
  // hasn't yet been appended to the firing log. Skip demotion for those
  // names to avoid staging BOTH .tmp (reinforcement) AND .tombstone
  // (demotion) for the same logical path — which the sweep's BR-7 guard
  // correctly refuses.
  const reinforcedSet = new Set(reinforcedNames);
  const out = [];
  for (const name of stale) {
    if (reinforcedSet.has(name)) continue; // Phase 2 just reinforced — fresh evidence
    const desc = active.find(p => p.name === name);
    if (!desc) continue;

    // Grace period: skip patterns whose age is ≤ gracePeriodDays. A pattern
    // just promoted has 0 firings by definition; demoting it before it has
    // had a chance to accrue evidence undoes the promotion gate's intent.
    // Use `<=` so a pattern that is exactly `gracePeriodDays` old gets the
    // full window; demotion fires from day `gracePeriodDays + 1` onward.
...
export async function runPrune(opts) {
  ...
  const summary = {
    ...
    journalArchived: journal.found && !journal.archiveCollision ? 1 : 0,
    journalArchiveCollision: Boolean(journal.archiveCollision),
...
async function stagePreimageSidecar(stagedPath, preimageContent) {
  ...
  // Ensure the parent dir exists; we may run before the matching .tmp is
  // staged (Codex R2 ordering fix), so mkdir-recursive is owned here.
...
export async function stagePrunePlan(args) {
  ...
  // Codex R2 ordering fix: write the sidecar FIRST, then the archive .tmp
  // last. A crash between the two leaves an orphan sidecar (harmless — sweep
  // ignores sidecars without a matching .tmp) instead of a guard-less .tmp
  // (dangerous — sweep would overwrite live archive without conflict check).
  async function stageArchiveAppend(archiveRel, blocks) {
    ...
    const stagedTmpPath = path.join(stagedRoot, `${archiveRel}.tmp`);
    // Write sidecar first using the planned .tmp path.
    const sidecarPath = await stagePreimageSidecar(stagedTmpPath, existingRaw);
    stagedFiles.push(sidecarPath);
    // Now write the archive .tmp; the sidecar is already in place to guard it.
  ...
  // Skip the entire step when archiveCollision was recorded — the live archive
  // diverges from the source and we don't want either side to clobber the
  // other. The dream-log will surface the collision; JJ resolves manually.
  if (plan.journal?.found && !plan.journal.archiveCollision) {
    ...
    // `target_missing` records whether the deletion target (the live source we
    // are about to remove via the sweep) is already gone; matches Phase-2
    // semantics where the field reports the file at `removed_path`.
    const sourceAbs = path.join(memoryRoot, plan.journal.sourceRel);
    let removalTargetMissing = false;
    try { await fs.access(sourceAbs); } catch (e) {
      if (e.code === 'ENOENT') removalTargetMissing = true; else throw e;
    }
    stagedFiles.push(await stageTombstone(
      ...
      tombstone(
        ...
        { targetMissing: removalTargetMissing },
      ),
    ));
  }
  ...
  for (const demo of (plan.demotions || [])) {
    ...
    // target_missing reports whether the deletion target (the live active
    // file we are tombstoning) is already gone at stage-time.
