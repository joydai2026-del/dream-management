...
function bumpField(body, key, value) {
  ...
  if (presentRe.test(body)) {
    // Match whole rest-of-line, then split off any inline `# comment`, so
    // the value is replaced atomically and a multi-word existing value
    // doesn't leak its tail into the bumped line. (Codex R2 finding —
    // mirrors the same fix in phase-3-prune.js#setFrontmatterField.)
    const replaceRe = new RegExp(
      `^([ \\t]*${escapeRegExp(key)}[ \\t]*:)([^\\r\\n]*)$`,
      'm',
    );
    return body.replace(replaceRe, (_, lead, rest) => {
