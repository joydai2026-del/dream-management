...
import { findRelativeDates, rewriteRelativeDates } from './phase-4-dates-contradictions.js';
...
async function checkArchiveBlockPresence({ dreamDir, today }) {
  const findings = [];
  const stagedRoot = path.join(dreamDir, 'staged');
  const snapshotDir = path.join(dreamDir, 'snapshot');
  ...
  for (const { sourceRel, archivePattern } of sources) {
    ...
    // R2-C: multiset semantics for duplicate H3 headings.
    //
    // Production-bug fix (2026-05-11): Phase 4 rewrites relative-date
    // phrases ("yesterday", "today", "last week") in hot-tier files
    // including corrections.md. A block whose heading contains a
    // relative-date phrase will have its heading REWRITTEN in post,
    // even when the block wasn't archived. Comparing pre-headings to
    // post-headings literally produces false-positive "missing" entries
    // for renamed blocks. The fix: apply the SAME Phase 4 date-rewrite
    // to each pre heading before lookup, so a renamed-only block matches
    // its post counterpart.
    function normalizeHeading(h) {
      return rewriteRelativeDates(h, today).content;
    }
    ...
    for (const block of preBlocks) {
      const normalizedHeading = normalizeHeading(block.heading);
      const remaining = postHeadingCounts.get(normalizedHeading) || 0;
      if (remaining > 0) {
        postHeadingCounts.set(normalizedHeading, remaining - 1);
        continue; // this occurrence (possibly renamed by Phase 4) stayed
      }
      missing.push(block);
    }
...
export async function runStageA(opts) {
  ...
  const checks = [
    ...
    ['archive_block_present', () => checkArchiveBlockPresence({ dreamDir, today })],
