...
// stage them for rewrite OR for delete (.tombstone). C10 enforces.
...
async function checkArchiveBlockPresence({ dreamDir }) {
  ...
  for (const { sourceRel, archivePattern } of sources) {
    ...
    const postHeadings = new Set(preBlocks.length > 0 ? extractHeadings(post) : []);
    const missing = preBlocks.filter(b => !postHeadings.has(b.heading));
...
async function checkAppendOnly({ dreamDir }) {
  ...
  // The sweep step honors tombstones by deleting the live file at the
  // logical path; an append-only target staged for delete is just as bad
  // as one staged for rewrite.
  for (const stagedRel of stagedRels) {
    let logical = null;
    let mode = null;
