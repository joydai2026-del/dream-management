...
// Sidecar-required prefixes: archive paths that REQUIRE a preimage sidecar
...
//
// Excluded by design: `archive/journals/<month>/<date>.md` and
// `archive/sessions/<month>/<file>.md` are first-write MOVES (Phase 3
// archives a learning-journal or session-log verbatim into a date-stamped
// path that won't exist before today). No preimage to verify; treat them
// as regular sources.
//
// `archive/sessions/session-index-<month>.md` IS an append (Phase 3 trims
// session-index and groups archived blocks per month). Distinct from the
// raw-session-log moves under `archive/sessions/<month>/<file>.md`.
...
export async function classifyStaged(stagedRoot) {
  ...
  for (const stagedRel of rels) {
    ...
    if (stagedRel.endsWith('.tmp')) {
      ...
      // Sidecar-required only for KNOWN-APPEND archive patterns (corrections
      // month-files + session-index month-files). First-write moves (journal
      // archives, session-log archives) under archive/ are categorized as
      // sources — no preimage to verify against.
