...
// Sidecar-required patterns: archive paths that REQUIRE a preimage sidecar
// because they're appends to an existing live archive (e.g.,
// `archive/corrections/2026-04.md` may already have content from prior
// runs, so we need to verify the live sha before overwrite).
const SIDECAR_REQUIRED_PATTERNS = [
  /^archive\/corrections\/\d{4}-\d{2}\.md$/,
  /^archive\/sessions\/session-index-\d{4}-\d{2}\.md$/,
];

// First-write-archive patterns: paths under archive/ that Phase 3 stages
// as fresh date-stamped MOVES (the target won't exist live pre-sweep).
// No preimage to verify; sweep treats them as sources but with the
// extra invariant that the live target MUST be absent at sweep time.
const FIRST_WRITE_ARCHIVE_PATTERNS = [
  /^archive\/journals\/\d{4}-\d{2}\/\d{4}-\d{2}-\d{2}\.md$/,
  /^archive\/sessions\/\d{4}-\d{2}\/[^/]+\.md$/,
];

const ARCHIVE_PREFIXES = ['archive/']; // kept for back-compat reads
...
export async function classifyStaged(stagedRoot) {
  ...
  for (const stagedRel of rels) {
    ...
    if (stagedRel.endsWith('.tmp')) {
      ...
      const requiresSidecar = SIDECAR_REQUIRED_PATTERNS.some(p => p.test(logicalRel));
      const isFirstWriteArchive = FIRST_WRITE_ARCHIVE_PATTERNS.some(p => p.test(logicalRel));
      const isUnderArchive = logicalRel.startsWith('archive/');

      if (requiresSidecar) {
        archives.push({ stagedRel, logicalRel, sidecarRel });
      } else if (isFirstWriteArchive) {
        // First-write archive moves (journals, raw session-log archives):
        // classify as sources but tag for the "live target absent at
        // sweep time" check. runSweep verifies that invariant.
        sources.push({ stagedRel, logicalRel, firstWriteArchive: true });
      } else if (isUnderArchive) {
        // Codex XPT-R1 #1: any archive/* path that doesn't match a
        // known append pattern OR a known first-write pattern is
        // structurally suspicious. A future Phase 3 that stages a
        // novel archive path would otherwise classify as a regular
        // source and skip sidecar verification — silent overwrite
        // risk. Fail closed.
        issues.push({
          severity: 'fail',
          message: `staged archive path matches no known sidecar/first-write pattern: ${logicalRel}`,
          path: stagedRel,
        });
      } else {
...
export async function runSweep(opts) {
  ...
  for (const s of classified.sources) {
    // First-write archive invariant: target MUST be absent at sweep
    // time (these paths are fresh date-stamped moves; a live target
    // means a prior run already swept the same logical path or a
    // concurrent writer landed there — either way, refuse to overwrite).
    if (s.firstWriteArchive) {
      const liveAbs = path.join(memoryRoot, s.logicalRel);
      let alreadyExists = false;
      try { await fs.access(liveAbs); alreadyExists = true; }
      catch (e) { if (e.code !== 'ENOENT') throw e; }
      if (alreadyExists) {
        result.conflicts.push({
          path: s.logicalRel,
          reason: 'first-write archive target already exists at sweep time',
        });
        result.aborted = true;
        return result;
      }
