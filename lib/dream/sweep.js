...
export async function classifyStaged(stagedRoot) {
  ...
  // BR-5: orphan preimage sidecars (no matching .tmp) are SUSPICIOUS — a
  // spoofed sidecar could pass Stage A then disappear, or a rename consumed
  // the .tmp but a stale sidecar remained. Either way, surface as a fail
  // finding rather than silently cleaning it up at sweep end.
  ...
  for (const sc of sidecars) {
    ...
    if (!archiveTmpSet.has(expectedTmp)) {
      issues.push({
        severity: 'fail',
        message: `orphan preimage sidecar (no matching .tmp): ${sc}`,
        path: sc,
      });
    }
  }
...
export async function runSweep(opts) {
  ...
  // STEP 2: archives first (leaf-first per § 2.1). BR-1: if ANY archive
  // rename fails, we MUST abort before source renames + tombstones.
  // Otherwise a partial-archive-commit followed by source-prune would
  // delete content from source that was never archived — the exact
  // "lost data" failure mode the leaf-first ordering is designed to
  // prevent.
  //
  ...
  // window from "all archives" (pre-flight) down to "one rename." Cost:
  // one extra readFile+hash per archive, already paid at pre-flight, so
  // the duplication is acceptable for the integrity guarantee.
  ...
  if (result.errors.length > 0) {
    // Preserve staged tree as evidence; do NOT proceed to sources or
    // tombstones; do NOT clean up. The next dream run's lock recovery +
    // staged-dir presence means the partial state is observable.
    result.aborted = true;
    return result;
  }

  // STEP 3: source rewrites. Same break-on-error rule: if a source rename
  // fails, abort before tombstones (a tombstone running while source is
  // half-renamed would compound the partial-commit damage).
  ...
  if (result.errors.length > 0) {
    // Tombstone phase failure: most archives + sources have already
    // committed, so we can't undo. Surface as aborted=true so the caller
    // knows the staged tree wasn't cleaned, and recovery tooling can
    // re-run remaining tombstones.
    result.aborted = true;
    return result;
  }
...
export async function finalizeAuditVerdicts(opts) {
  ...
  if (dle !== null) {
    ...
    // F4: idempotent footer. Bracket the audit summary with sentinel
    // comments so re-running this function (e.g., after a Stage B re-run
    // following a transient failure) replaces the prior footer instead of
    // appending another. Without this, every re-run doubled the footer
    // length and silently corrupted the human-readable archive.
    ...
    const trimmed = sentinelRe.test(updated)
      ? updated.replace(sentinelRe, '')
      : updated.replace(/\s*$/, '');
