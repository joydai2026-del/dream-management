 ...
 *   3. Token-majority fallback: ≥ min(2, N) distinct kebab-tokens of the
 *      identifier present with word-boundary matches in the haystack.
 *      For 1-token identifiers rung 3 is a no-op (would duplicate rung 1).
 *      For 2+-token identifiers it catches prose like "DOM drift broke" →
 *      matches `external-dom-drift-llm-default` even without a curated
 *      trigger_phrase. Patterns SHOULD still declare trigger_phrases when
 *      they have specific shorthand; this fallback covers the general case.
 *
...
export function applyPromotionGates(candidates, opts = {}) {
  ...
  for (const c of (candidates || [])) {
    ...
    const journalMentions = Number.isFinite(Number(c.journalMentions))
      ? Number(c.journalMentions) : 0;
