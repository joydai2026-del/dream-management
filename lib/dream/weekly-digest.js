...
function summarize(events) {
  ...
  for (const { date, evt } of events) {
    ...
    for (const c of cs) {
      // Reality-checker final #5: use the iterating dream-pass date, not
      // run_id. run_id is the wall-clock when finalize wrote — at 3:45am
      // it's the next calendar day, so contradictions get mislabeled.
...
function renderDigest({ today, windowStart, windowEnd, events, summary }) {
  ...
  for (const { date, evt } of events) {
    ...
    if ((r.patterns_reinforced || []).length > 0) {
      // Reality-checker final #6: guard against missing fields. event.json
      // shape allows {pattern, sightings_after}; older runs may emit
      // {pattern} only. Skip undefined fields rather than rendering them.
      const examples = r.patterns_reinforced.slice(0, 3)
        .map(p => p.sightings_after !== undefined
          ? `${p.pattern}→${p.sightings_after}`
          : String(p.pattern || '?'))
        .join(', ');
