...
function parseScalars(body) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inline.startsWith('[') && inline.endsWith(']')) {
      ...
      out[key] = inside === '' ? [] : inside.split(',').map(s => stripQuotes(s.trim()));
      continue;
    }
    out[key] = coerce(stripQuotes(inline));
  }
  return out;
}
...
export const _internals = { parseScalars, FRONTMATTER_RE };
